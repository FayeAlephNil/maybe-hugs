...
(define (make-cutie some-name pronoun1 pronoun2 pronoun3)
  `((name . ,some-name) (pronouns . (,pronoun1 . (,pronoun2 . ,pronoun3))) (listen? . ,#f) (hug? . ,#f) (give-advice? . ,#f)))

(define (listening do? pronouns)
  (if do?
      (string-append " appreciates you listening to " (cadr pronouns))
...
(define (hugging do? pronouns)
  (if do?
      ...
      (string-append " and respecting " (cddr pronouns) " boundaries")))
